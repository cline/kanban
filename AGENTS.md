...
- For Pi task integration, preserve the real Pi PTY/TUI experience. Do not switch task sessions to `--mode json` just to get structured events, or the terminal UX stops matching Codex/Claude. The working pattern is: launch real interactive `pi` in the PTY, give each task its own `--session-dir`, watch Pi's session `.jsonl` file in parallel for Kanban transitions/activity, and disable task auto-restart on clean exit to avoid answer loops. Add/update tests for these invariants when touching Pi plumbing.
- Pi reliability gotcha: detached best-effort hook subprocesses were too flaky for fast Pi completions. The robust pattern is to ingest Pi watcher events directly in-process from the Pi wrapper, queue them serially, and keep only one narrow fallback: on clean Pi exit, if the task is still running and the latest Pi session log clearly contains a final answer, reconcile it to review from the same session-log source of truth. Avoid stacking multiple unrelated fallback mechanisms.
- Pi readiness gotcha: do not infer readiness from `pi --mode json`; in logged-out/misconfigured states it can just hang. The useful probe is `pi --mode rpc --no-session` with a tiny prompt request. Also, treat **any** `prompt` response with `success: false` as authoritative not-ready even if Pi later emits another `success: true` response for the same request id; we observed that duplicate-response quirk in real runs.
- Product/UX decision for Pi readiness failures: do **not** block task start or bounce the task back out of `in_progress`. Launch the real Pi terminal anyway so the user can complete `/login` or other setup inside that terminal, keep the task running, and surface the problem via `warningMessage`/a card tag like `User action needed`. Clear that warning once the user sends new terminal input.
- Pi follow-up UX: keep a separate `needsManualPromptResend` session flag when startup failed readiness but the original task prompt still needs to be resent manually after login. Do **not** tie the resend button to `warningMessage` alone, because the warning is cleared as soon as the user types `/login`. Also, when the UI triggers "Send original prompt", route it through the backend `sendTaskSessionInput` path with `preferTerminal: false` so Kanban can clear the resend flag on the persisted session summary instead of bypassing state updates through the local terminal controller.
