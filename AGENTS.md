...
- **Deferred startup input timing for TUI agents**: When detecting that a TUI agent is ready to receive its initial prompt, look for something that only appears AFTER `app.run()` (i.e., after the TUI switches the PTY to raw mode). For Hermes, the welcome banner "Welcome to Hermes Agent! Type your message..." is printed BEFORE `app.run()`. Triggering on it sends the deferred input while the PTY is in cooked mode — `ICRNL` converts `\r` to `\n` (ControlJ), and in SSH sessions Hermes binds ControlJ to insert-newline rather than submit. The fix: detect `❯` (the prompt_toolkit input prompt symbol) which only renders after `app.run()` has started and raw mode is active. In general, any `hasXxxStartupUiRendered` trigger should fire on TUI-rendered output, not on pre-TUI static banners.
