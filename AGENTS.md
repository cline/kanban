...
- `projects.list` is a latency-sensitive path: avoid loading full workspace state (`loadWorkspaceState`) just to count tasks, because it triggers sync git capture and can add ~1s to project-switch/snapshot flows on multi-project setups. Use direct board reads by `workspaceId` and coalesce/batch repeated `projects_updated` broadcasts from task-session churn.
