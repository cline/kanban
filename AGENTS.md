...
- Codex new-task startup now uses one global `codex app-server` for the whole Kanban runtime and routes fresh Codex task turns through that shared host; do not reintroduce per-task Codex CLI spawn on the hot start path. Trash restore still intentionally falls back to the older PTY path until exact thread persistence lands, so if you touch Codex restore logic, treat that split as deliberate, temporary architecture rather than dead code.
