...
describe.sequential("runtime-config auto agent selection", () => {
	it("selects agents using the configured priority order", () => {
		expect(pickBestInstalledAgentIdFromDetected(["cursor", "codex", "droid"])).toBe("cursor");
		...
		expect(pickBestInstalledAgentIdFromDetected(["claude", "cursor", "codex", "cline"])).toBe("claude");
