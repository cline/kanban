...
- Projects can also enable an autonomous Agent Reviewer for cards in \`review\`. When that policy is on, a completed task entering \`review\` may stay pinned there while a reviewer agent writes \`CODE_REVIEW.md\`, asks the original agent for fixes, and repeats until the review passes or the max rounds are exhausted. Users can also manually trigger the same reviewer from a review card's context menu for one ticket without changing the project default.
