...
function renderLinearSetupGuidanceForAgent(agentId: RuntimeAgentId | null): string {
	switch (agentId) {
		...
		case "prime":
			return "- If Linear MCP is not available in the current agent (Prime Agent), suggest running: `prime-agent mcp add -name linear --url https://mcp.linear.app/mcp --scope user`";
		default:
