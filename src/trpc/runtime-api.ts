	...
	return {
		...
		startTaskSession: async (workspaceScope, input) => {
			try {
				...
				let usePrimeAcpPath = effectiveAgentId === "prime" && isPrimeAcpEnabled() && Boolean(deps.getScopedPrimeAcpTaskSessionService);
					...
					if (!isHomeAgentSessionId(body.taskId)) {
						...
						if (!summary) {
							if (isPrimeAcpEnabled() && deps.getScopedPrimeAcpTaskSessionService) {
								try {
									const primeService = await deps.getScopedPrimeAcpTaskSessionService(workspaceScope);
									const pSummary = await primeService.sendTaskSessionInput(body.taskId, body.text, body.images);
									if (pSummary) {
