...
export class PrimeAcpClientImpl {
	...
	async readTextFile(params: {
		...
		try {
			...
			throw new Error(`readTextFile failed for ${params.path}: ${msg}`);
		}
	}
	...
	async createTerminal(params: {
		...
		const terminal: PrimeAcpManagedTerminal = {
			id: terminalId,
			process: proc,
			output: "",
			truncated: false,
	...
	async terminalOutput(params: { sessionId: string; terminalId: string }): Promise<{
		output: string;
		truncated: boolean;
		...
		return {
			...
			...(t.exited ? { exitStatus: { exitCode: t.exitCode, signal: t.signal } } : {}),
		};
	}
	async waitForTerminalExit(params: {
		...
		await new Promise<void>((resolve) => {
			t.process.once("close", () => resolve());
			t.process.once("error", () => resolve());
		});
	...
	async releaseTerminal(params: { sessionId: string; terminalId: string }): Promise<Record<string, never>> {
		...
		if (!t.exited) {
			try {
				...
				setTimeout(() => {
					try {
						if (!t.exited) t.process.kill("SIGKILL");
					} catch {}
				}, 2000);
			} catch {}
		}
