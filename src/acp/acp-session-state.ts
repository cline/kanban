...
export function updateSummary(entry: PrimeAcpTaskSessionEntry, patch: Partial<RuntimeTaskSessionSummary>): RuntimeTaskSessionSummary {
  entry.summary = { ...entry.summary, ...patch, updatedAt: now() };
  return cloneSummary(entry.summary);
}
export function createMessage(taskId: string, role: PrimeAcpMessage["role"], content: string, images?: RuntimeTaskImage[]): PrimeAcpMessage {
  return {
...
export function createMessageWithMeta(taskId: string, role: PrimeAcpMessage["role"], content: string, meta: PrimeAcpMessage["meta"], images?: RuntimeTaskImage[]): PrimeAcpMessage {
  return { ...createMessage(taskId, role, content, images), meta };
}
function updateMessageInEntry(entry: PrimeAcpTaskSessionEntry, messageId: string, updater: (m: PrimeAcpMessage) => PrimeAcpMessage): PrimeAcpMessage | null {
...
export function upsertToolCall(entry: PrimeAcpTaskSessionEntry, taskId: string, toolCallId: string, title: string, status?: string | null, content?: unknown, kind?: string | null): PrimeAcpMessage {
  ...
  if (existingId) {
    const updated = updateMessageInEntry(entry, existingId, (m) => ({
      ...m,
      content: toolContent,
      meta: { ...(m.meta ?? {}), toolName: kind ?? m.meta?.toolName ?? null, toolCallId, messageKind: status ?? null },
    }));
    if (updated) return updated;
  }
  const msg = createMessageWithMeta(taskId, "tool", toolContent, { toolName: kind ?? null, toolCallId, messageKind: status ?? null });
  entry.messages.push(msg);
...
function formatToolContent(title: string, status?: string | null, content?: unknown): string {
  ...
  if (content) {
    ...
    else { try { body = JSON.stringify(content, null, 2); } catch { body = String(content); } }
  }
...
export function createSessionEntry(taskId: string): PrimeAcpTaskSessionEntry {
  return { summary: createDefaultSummary(taskId), messages: [], activeAssistantMessageId: null, activeReasoningMessageId: null, toolMessageIdByToolCallId: new Map() };
}
