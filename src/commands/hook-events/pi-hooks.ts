...
export function buildPiWrapperChildArgs(agentArgs: string[], sessionDir: string): string[] {
	...
	const hasSessionDir = childArgs.some((arg) => arg === "--session-dir" || arg.startsWith("--session-dir="));
	if (!hasSessionDir) {
...
export async function runPiWrapperSubcommand(
	...
	const spawnSpec = shouldUseWindowsCmdLaunch(wrapperArgs.realBinary, process.platform, childEnv)
		? {
				binary: resolveWindowsComSpec(childEnv),
				args: buildWindowsCmdArgsArray(wrapperArgs.realBinary, childArgs),
			}
		: { binary: wrapperArgs.realBinary, args: childArgs };
	const child = spawn(spawnSpec.binary, spawnSpec.args, {
		stdio: "inherit",
		env: childEnv,
	});
