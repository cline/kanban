...
export function createRuntimeStateHub(deps: CreateRuntimeStateHubDependencies): RuntimeStateHub {
	...
	return {
		...
		close: async () => {
			...
			for (const unsubscribe of primeSummaryUnsubscribeByWorkspaceId.values()) {
				try {
					unsubscribe();
				} catch {}
			}
			...
			for (const unsubscribe of primeMessageUnsubscribeByWorkspaceId.values()) {
				try {
					unsubscribe();
				} catch {}
			}
